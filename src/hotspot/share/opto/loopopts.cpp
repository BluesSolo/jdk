...
// Clone "n" for uses that are outside of 'loop'. Return the number of cloned nodes.
uint PhaseIdealLoop::clone_for_use_outside_loop(IdealLoopTree* loop, Node* n, Node_List& worklist, Node_Array& initial_outside_uses_map) {
  uint cloned = 0;
...
Node* PhaseIdealLoop::get_clone_for_outside_use(const Node* n, Node* outside_use, Node_Array& outside_uses_map, Node_List& clones_of_n_for_use) {
  ...
  // Have we already cloned n for the initial_outside_use chain? If so directly use it and do not create another clone for it.
  Node* n_clone = clones_of_n_for_use[initial_outside_use->_idx];
  if (n_clone == NULL) {
    // We have not cloned n, yet, for the chain of nodes belonging to the initial outside of tappeahe loop use initial_outside_use.
    ...
    if (!n->is_CFG() && has_use_in_set(n, not_peel)) {
      ...
      if (!has_use_internal_to_set(n, peel, loop)) {
        ...
        if (n->in(0) == NULL && !n->is_Load() && !n->is_CMove()) {
          cloned_for_outside_use += clone_for_use_outside_loop(loop, n, worklist, outside_uses_map);
          C->print_method(PHASE_DEBUG, 3);
          sink_list.push(n);
          peel.remove(n->_idx);
          not_peel.set(n->_idx);
          peel_list.remove(i);
...
#ifndef PRODUCT
  if (TracePartialPeeling && exceeded) {
